...
namespace END.Config.Crypto;
    ...
    public class Aes256Cipher
    {
        ...
        private const int HashSize = 32; // size in bytes
        ...
        private static string DeriveSymmetricKey(string input)
        {
            var inputBytes = Encoding.UTF8.GetBytes(input);
        ...
        private static (string cipherText, string nonce) GetCipherParams(string cipher)
        {
            ...
            var cipherParams = cipher.Split('|');
            var cipherText = cipherParams[0];
            var nonce = cipherParams[1];
        ...
        public string Decrypt(string secretKey, string cipherText)
        {
            ...
            var key = DeriveSymmetricKey(secretKey);
